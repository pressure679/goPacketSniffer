...
// Keywords: Confidentiality, integrity, accessibility
package main
import (
	"fmt"
...
func main() {
	passphrase  := "10zZz10ZZzZ" // AP password
	ssid        := "Netgear 2/158" // SSID
	a           := "Pairwise key expansion" // per definition generated by a PRF - custom or standardized, e.g HMAC or MD5
	apmac       := "001e2ae0bdd0" // MAC address of the AP (Access Point)
	clientmac   := "cc08e0620bc8" // MAC address of the client
	...
	snonce      := "60eff10088077f8b03a0e2fc2fc37e1fe1f30f9f7cfbcfb2826f26f3379c4318" // made randomly, e.g by a chosen PRNG (PseudoRandom Number Generator)
	...
	pmk := pbkdf2.Key([]byte(passphrase), []byte(ssid), 4096, 32, sha1.New) // salt
	ptk := CustomPRF512(string(pmk), a, b, ssid) // 512 bit key, byte 32:48 is used as encryption key (Temporal Encryption Key, shown later)
	...
	file := flag.String("file", "", "the file to read data from to encrypt with a simulation of assymmetric keys like in WPA - using AES, SHA1, and MD5 - the method used ix 16 byte PTK[32:48] xor")
	flag.Parse()
...
func CustomPRF512(key, a, b, ssid string) []byte {
	...
	for i := 0; i < 5; i++ {
		...
		passwordstr := "Pairwise key expansion" + key + a + string(nullbyte) + b + string(i)
